...
class Main(basetask.BaseTask):
    name = "用所有DNS获取域名ip,cname.DNSPython包"
    inputcolums = {'domain':{'name': "domain", "value": 'domain'}}
    args={
               'outtable':{'name': 'outtable', 'value': "", 'alisaname': "结果输出表",'formator':'domainresult_{now}'}
    }
    ...
    async def getresult(self):
        updatesql=f"insert into {self.args['outtable']['value']} (dnsip,domain,ip,cname) values (%s,%s,%s,%s)"
