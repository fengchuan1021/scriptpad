...
class Main(basetask.BaseTask):
    name = "用所有DNS获取域名CNAME"
    inputcolums = {'domain':{'name': "domain", "value": 'domain'}}
    args={
               'outtable':{'name': 'outtable', 'value': "", 'alisaname': "结果输出表",'formator':'domaincnameresult_{now}'}
    }
    ...
    async def getresult(self):
        updatesql=f"insert into {self.args['outtable']['value']} (dnsip,domain,cname) values (%s,%s,%s)"
