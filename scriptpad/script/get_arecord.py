...
class Main(basetask.BaseTask):
    ...
    args={
               'outtable':{'name': 'outtable', 'value': "", 'alisaname': "结果输出表",'formator':'domain_arecord_result_{now}'}
    }
  ...
  `ip` int(11) UNSIGNED NULL DEFAULT NULL COMMENT 'ip地址',
  ...
  UNIQUE INDEX `index_ip`(`ip`, `domain`) USING BTREE,
  INDEX `domain_idx`(`domain`) USING BTREE
    ...
    async def getresult(self):
        updatesql=f"insert ignore into {self.args['outtable']['value']} (domain,ip) values (%s,%s)"
        ...
        while 1:
            ...
            if result:
                ...
                for i in result:
                    ...
                    rows.append([domain,int(ipaddress.ip_address(ip))])
                if rows:
