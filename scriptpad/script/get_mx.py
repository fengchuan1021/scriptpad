...
class Main(basetask.BaseTask):
    ...
    args={
               'outtable':{'name': 'outtable', 'value': "", 'alisaname': "结果输出表",'formator':'domain_mx_result_{now}'}
    }
    ...
    async def startremote(self):
        dic = {"listname": f"{self.tasklist}",'qtype':"MX"}
    ...
    async def getresult(self):
        updatesql=f"insert ignore into {self.args['outtable']['value']} (domain,mx_record) values (%s,%s)"
