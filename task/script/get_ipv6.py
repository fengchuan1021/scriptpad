...
class Main(basetask.BaseTask):
    ...
    args = {
        'outtable': {'name': 'outtable', 'value': "", 'alisaname': "结果输出表", 'formator': 'ljz_ipv6_zxinc_{now}'}
    }

    async def init(self):
        tablesql = f'''CREATE TABLE IF NOT EXISTS `{self.args['outtable']['value']}`(
    `id` int(11) NOT NULL AUTO_INCREMENT,
    `minipv6` varchar(255) CHARACTER SET utf8 COLLATE utf8_bin NULL DEFAULT NULL,
    `start` varchar(255) CHARACTER SET utf8 COLLATE utf8_bin NULL DEFAULT NULL,
    `end` varchar(255) CHARACTER SET utf8 COLLATE utf8_bin NULL DEFAULT NULL,
    ...
    async def startremote(self):
        ...
        cmd = f"sudo /home/ant/conda/bin/python /home/ant/lg/scripts/ipv6_data.py '{json.dumps(dic)}'"
        print(cmd)
    ...
    async def getresult(self):
        ...
        updatesql = f"insert into {self.args['outtable']['value']} (id,minipv6,start,end,location,country,local) values (%s,%s,%s,%s,%s,%s,%s)"
        print(updatesql)
        while 1:
            ...
            if result:
                ...
                rows = []
                for i in result:
                    rows.append(i.split('_',6))  # id_infos  just split into 2 length array.
                if rows:
