...
class Main(basetask.BaseTask):
    ...
    args={
               'outtable':{'name': 'outtable', 'value': "", 'alisaname': "结果输出表",'formator':'domain_ip6address_result_{now}'}
    }
    ...
    async def startremote(self):
        dic = {"listname": f"{self.tasklist}",'qtype':"AAAA"}
    ...
    async def init(self):
            tablesql = f'''CREATE TABLE IF NOT EXISTS `{self.args['outtable']['value']}`  (
              ...
              `ip6address` varchar(64) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
              ...
              UNIQUE INDEX `index_ip`(`ip6address`, `domain`) USING BTREE,
              INDEX `domain_idx`(`domain`) USING BTREE
    ...
    async def getresult(self):
        updatesql=f"insert  ignore into {self.args['outtable']['value']} (domain,ip6address) values (%s,%s)"
        #updatesql = f"insert into {self.dbconfig['sourcetable']} (id,{self.outputcolums['ip6address']['value']}) values (%s,%s) ON DUPLICATE KEY UPDATE {self.outputcolums['ip6address']['value']}=VALUES({self.outputcolums['ip6address']['value']})"
