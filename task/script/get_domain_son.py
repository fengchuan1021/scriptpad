...
class Main(basetask.BaseTask):
    name = "探测域名的子域名"
    atmobiles = ['刘金周']
    inputcolums = {'url': {'name': "url", "value": 'url'}}
    outputcolums = {'flag': {'name': "flag", 'value': 'flag',
    ...
    async def startremote(self):
        ...
        cmd = f"sudo /home/ant/conda/bin/python /home/ant/lg/scripts/domain_son.py '{json.dumps(dic)}'"
        print(cmd)
        await self.remoterun(cmd)

    async def sendtask(self):
        sql = f"""select id,{self.inputcolums['url']['value']} from {self.dbconfig['sourcetable']} {self.dbconfig['condition']} order by id desc limit 1"""
        print(sql)
        ...
        while sqlid <= self.maxid:
            if int(await self.redis.llen(self.tasklist)) > 10000:
                ...
                continue
            sql = f"""select id,{self.inputcolums['url']['value']} from {self.dbconfig['sourcetable']} where id between {sqlid} and {sqlid+10000-1} {self.dbconfig['condition'].replace('where','and',1)}"""
    ...
    async def getresult(self):
        ...
        updatesql = f"insert into {self.dbconfig['sourcetable']} (id,{self.outputcolums['flag']['value']} ) values (%s,%s) ON DUPLICATE KEY UPDATE {self.outputcolums['flag']['value']}=VALUES({self.outputcolums['flag']['value']})"
        print(updatesql)
        while 1:
